---
name: get-e.workflow.plan-story
...
1. Read `.ai/context/MEMORY.md` (if it exists). Identify entries relevant to the current scope. Include applicable lessons in all delegations.
...
Create `<issue-folder>/summary.md` by following the `get-e.plan-summary` skill.

Output the summary to the human stakeholder and ask for final approval.

### Step 6: Transition

On approval, proceed to implementation using the `get-e.workflow.implement-story` skill.
...
Document lessons learned in `.ai/context/MEMORY.md` using the `get-e.memory-management` skill:
